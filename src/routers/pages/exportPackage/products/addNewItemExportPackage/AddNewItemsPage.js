...
const AddNewItemExportPackagePage = () => {
  ...
  const {updatedDataExportPage} = useAppContext();
  ...
  const {numberVoteShowingExportPage} = useAppContext();
  const {setIsReloadDataExportVote} = useAppContext();

  const options = ["---Lựa chọn---", ...new Set (rowDataDefault.map(item => item.productType))];

  //Dữ liêu MQTT
  const { isConnectedRefExportPages } = useAppContext();
  const { isconnectedMQTTBrokerExportPage } = useAppContext();
  ...
  const switchSelectedOptionFillDataScannerDevices = useCallback (() => {
    switch (selectedOptionFillDataScannerDevices) {
      case 'default':
        setUpdatedDataExportPage((pre) => {
          const previousData = Array.isArray(pre) ? pre : [];
          let updatedData = [...previousData]; 
          const matchedItemInDefaultList = updatedData.find(
            (item) => item?.code === messageMQTTBrokerExportPage?.message
          );
      
          if (matchedItemInDefaultList) {
            const existingItemIndex = updatedData.findIndex(item => item.code === messageMQTTBrokerExportPage?.message);
            ...
            const newItem = {
              ...
              code: messageMQTTBrokerExportPage?.message,
  ...
  }, [messageMQTTBrokerExportPage, selectedOptionFillDataScannerDevices, rowDataDefault, setUpdatedDataExportPage]);

  const checkOpenDataBySearchFromVotePageButton = useCallback(() => {
    if (isOpenAddVotesPageBySearchExportPage === false) {
      if (isconnectedMQTTBrokerExportPage) {
        if (messageMQTTBrokerExportPage.length === 0) return null;
    ...
    if (check.includes(numberVoteShowingExportPage)) {
      const matchedItem = dataFetch.find(
        item => item?.voteFillInfo?.exportPackageVote?.numberVote === numberVoteShowingExportPage
      );
  
      const dataToUpdate = matchedItem?.merchandise || [];
      const dataFillFormCreatVoteInfomation = matchedItem?.voteFillInfo?.exportPackageVote || [];
      
      setUpdatedDataExportPage(dataToUpdate.length > 0 ? dataToUpdate : rowDataDefault.map(row => ({
        ...row,
        quantityAdd: '',
        totalPrice: '',
      })));
  ...
  useEffect(() => {
    setData(updatedDataExportPage);
  }, [updatedDataExportPage]);
  ...
  const handleInputChange = debounce((event) => {
    ...
    if (checkInput.length === 0) {
      if (updatedDataExportPage.length > 0) {
        setData(updatedDataExportPage);
      }
      else return '';
    }
    const result =  updatedDataExportPage.filter(item => 
                                                  item.code.toLowerCase().includes(checkInput) ||
                                                  item.productName.toLowerCase().includes(checkInput));
    setData(result.length > 0 ? result : updatedDataExportPage);
  }, 300);

  const [isOpenListVotePage, setIsOpenListVotePage] = useState(false);
  const {isShowButtonBackProductExportPage} = useAppContext();
  const {setIsShowButtonBackProductExportPage} = useAppContext();
  const {isShowButtonBackVoteListExportPage} = useAppContext();

  const handleOpenListVotePage = () => {
    setIsOpenListVotePage(!isOpenListVotePage);
    setIsShowButtonBackProductExportPage(false);
  }

  const handleCloseListVotePage = () => {
    setIsOpenListVotePage(!isOpenListVotePage);
    setIsShowButtonBackProductExportPage(!isShowButtonBackProductExportPage);
  }
          ...
          return (
            <input
              ...
              onChange={(e) => {
                ...
                if (inputValue === "" || (Number(inputValue) > 0 && Number(inputValue) <= row.original.quantity)) {
                  setTempValue(inputValue === "" ? null : Number(inputValue));
                }
              }}
              onBlur={() => {
                if (tempValue !== null && tempValue > 0 && tempValue <= row.original.quantity) {
  ...
  useEffect(() => {
    if (shouldPrint && printData.maPhieu) {
      ...
      printWindow.document.write(`
        <html>
          ...
          <body onload="window.print(); window.close();">
            ...
            <h2>Mã phiếu: ${printData.maPhieu}</h2>
            <p style="text-align: center; font-style: italic;">${printData.ngay}</p>

            <!-- Thông tin chung -->
            <div class="info">
              <p><b>Chi nhánh nhập:</b> ${printData.chiNhanh}</p>
              <p><b>Nhà cung cấp:</b> ${printData.nhaCungCap}</p>
              <p><b>Địa chỉ:</b> ${printData.diaChi}</p>
            </div>
            ...
            <div class="totals">
              <p><b>Tổng số lượng hàng:</b> ${printData.tongSoLuong}</p>
              <p><b>Tổng tiền hàng:</b> ${printData.tongTienHang}</p>
            </div>
  ...
  const handleScanningDataMode = async () => {
  ...
  const handleSearchTypeProductTask = () => {
    setData(updatedDataExportPage.filter(item => item.productType.toLowerCase().includes(selectedValue.toLowerCase())));
  };
  ...
  const handleSaveDataAllForm = async () => {
    ...
    try {
      ...
      const functionName = 'add_voteExportPackage_INLIST_FC';
      ...
      if (response.success === true) {
        notifySuccess();
        setTimeout(() => {
          setIsReloadDataExportVote(true);
        }, 3000);
      };
  ...
  const handleChangeFillDataScannerDevices = (event) => {
    setSelectedOptionFillDataScannerDevices(event.target.value);
    if (isconnectedMQTTBrokerExportPage) {
    ...
    <div className={styles.containerProducts}>
      {isOpenListVotePage ? (
        <div className={styles.newShowListVotePageContainer}>
          {isShowButtonBackVoteListExportPage && (
            ...
            <div className={styles.addProduct}>
              <div className={styles.searchProductLayerHeader}>
                ...
                <div className={styles.activeProductIcon}>
                  ...
                  <div className={styles.methodFillProductIcon} onClick={handleScanningDataMode}>
                            ...
                            <div className={styles.AddNewItemPageBodyList}>
                              <div className={styles.setupDataShowFromScannerDevices}>
                                ...
                                <div className={styles.setupDataShowFromScannerDevicesChoose}>
                                  ...
                                  <select
                                    id="dataTypeSelector"
                                    value={selectedOptionFillDataScannerDevices}
...
export default AddNewItemExportPackagePage;
